export interface AtlasPublicReadinessInput {
  ...
  minorSafetyReviewRequired?: boolean;
...
const REQUIRED_BOOLEAN_KEYS: Array<keyof Omit<AtlasPublicReadinessInput, "geographicScope" | "minorSafetyReview" | "minorSafetyReviewRequired">> = [
  "database",
  "externalAi",
  "transactionalEmail",
...
export function evaluateAtlasPublicReadiness(input: AtlasPublicReadinessInput): AtlasPublicReadinessResult {
  ...
  if (input.minorSafetyReviewRequired && !input.minorSafetyReview) missing.push("minorSafetyReview");
  ...
  return { ready: blockers.length === 0, scopeReady, missing, blockers };
}
...
function audienceEnabled(value: string | undefined, audience: string): boolean {
  return (value || "adult")
    .split(",")
...
export function getAtlasPublicReadinessFromEnv(env: NodeJS.ProcessEnv): AtlasPublicReadinessResult {
  return evaluateAtlasPublicReadiness({
    ...
    transactionalEmail: Boolean(env.ATLAS_EMAIL_FROM && (env.BREVO_API_KEY || env.RESEND_API_KEY)),
    ...
    minorSafetyReviewRequired: audienceEnabled(env.ATLAS_LAUNCH_AUDIENCES, "adolescent"),
